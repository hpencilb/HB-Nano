...
def get_host_ip():
    ...
    try:
        if any(net.family is socket.AF_INET for net in network['wlan0']):
            return network['wlan0'][0].address
    except KeyError:
        try:
            if any(net.family is socket.AF_INET for net in network['eth0']):
                return network['eth0'][0].address
            return 'unavailable'
        except KeyError:
            return 'unavailable'
...
if __name__ == "__main__":
    ...
    while True:
        with canvas(device) as draw:
            ...
            layout(draw, (status_x + 60, status_y + 14), 10, 'D:', f'{humanbytes(download_speed)}'.replace('.', ' .'),
                   font2)
